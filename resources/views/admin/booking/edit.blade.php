...
<div class="container-fluid py-5">
    <div class="container">
        <h1 class="display-5 mb-4">تعديل الحجز #{{ $booking->id }}</h1>
        ...
        <form action="{{ route('bookings.update', $booking->id) }}" method="POST">
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="date" class="form-control" id="booking_date" name="booking_date" value="{{ $booking->booking_date->format('Y-m-d') }}" required>
                </div>
                
                <div class="col-md-6 mb-3">
                    <label for="payment_method" class="form-label">طريقة الدفع</label>
                    <select class="form-select" id="payment_method" name="payment_method" required>
                        <option value="online" {{ $booking->payment_method == 'online' ? 'selected' : '' }}>دفع إلكتروني</option>
                        <option value="on_spot" {{ $booking->payment_method == 'on_spot' ? 'selected' : '' }}>دفع عند الوصول</option>
                    </select>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select" id="status" name="status" required>
                        ...
                        <option value="cancelled" {{ $booking->status == 'cancelled' ? 'selected' : '' }}>ملغية</option>
                    </select>
                </div>
